  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 4 }}>
        <Avatar src={artist.image_url} sx={{ width: 120, height: 120, mr: 3 }} />
        <Box>
          <Typography variant="h2" sx={{ color: '#fff', fontWeight: 800 }}>{artist.name}</Typography>
        </Box>
      </Box>
      <Typography variant="h5" sx={{ color: '#fff', fontWeight: 700, mb: 2 }}>Phổ biến</Typography>
      {songs.map((song, idx) => (
        <Box key={song._id} sx={{ display: 'flex', alignItems: 'center', py: 1, borderBottom: '1px solid #222', cursor: 'pointer', '&:hover': { bgcolor: 'rgba(30,215,96,0.05)' } }} onClick={() => {
