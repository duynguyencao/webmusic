...
const History = ({ user, setCurrentSong, onUpdatePlaylist }) => {
    ...
    const fetchHistory = async () => {
        try {
            const response = await fetch(`http://100.98.198.23:8080/api/history/${user._id}`);
    ...
    const handleRemoveFromHistory = async (songId) => {
        try {
            const response = await fetch('http://100.98.198.23:8080/api/history', {
                method: 'DELETE',
                headers: {
            ...
            <Fade in={isLoaded} timeout={800}>
                <Box>
                    <Box sx={{ mb: 6, position: 'relative', zIndex: 1 }}>
                        ...
                        >
                            Những bài hát bạn đã nghe gần đây
                        </Typography>
                    </Box>
                        ...
                        <Table>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell sx={{ color: '#b3b3b3', borderBottom: '1px solid rgba(255,255,255,0.1)', py: 2 }}>Xóa</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {songs.map((song, idx) => (
                                    ...
                                    >
                                        ...
                                        <TableCell>
                                            <IconButton 
                                                onClick={() => handleRemoveFromHistory(song._id)} 
                                                sx={{ 
                                                    color: '#e53935',
                                                    transition: 'transform 0.2s ease',
                                                    '&:hover': {
                                ...
                                {songs.length === 0 && (
                                    <TableRow>
                                        <TableCell colSpan={6} sx={{ color: '#b3b3b3', textAlign: 'center', py: 4 }}>
                                            Chưa có bài hát nào trong lịch sử nghe nhạc
                                        </TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
